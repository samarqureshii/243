...
// note that y is limited from 0 -> 236(becaues of 3 b 3 box)
    ...
    int boxes[8][4] =   {{x1, y1, direction_x1, direction_y1}, // pixel 1
    ...
    const int directions[4][2] = { {-1, -1}, //up and left 
                                    {1, -1}, //up and right
                                    {-1, 1}, //down and left
                                    {1, 1}}; //down and right
    ...
    *(pixel_ctrl_ptr + 1) = (int) &Buffer2;
    ...
    while (1){
        ...
        // code for generating a new direction when a pixel hits the edge of the screen 
...
void generateNewDirection(){ //call this function when the x or y go out of bounds 
    ...
    for(){ //
        for(){
            if(){ //in the case where a pixel has gone out of bounds  
                ...
                // and update the direction in the array 
...
void drawLines(){
    // draw all lines for every single pixel
}
