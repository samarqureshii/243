# Have the program search through 15 numbers (rather than 8), in which you choose those 15 numbers, and they should be unique - 
# i.e. none of them are the same). The numbers should be small enough that they can be represented in 10 or few bits, 
# as we will want to display them on the LEDs on the DE1-SoC.
...
/* keep largest number so far in r11 */
	...
	# this register will be used to keep track of the current biggest number when we iterate through the numbers label
...
loop: subi r9, r9, 1 # decrements a counter in r9 which initially contains the number of elements in the array
	   ...
	   # atp, we still have not found the biggest number 
