// takes input from microphone jack on DE1-SoC board, and connects it to the output speaker jack on the board
//using audio input and output system that collects samples from the input FIFO of the audio interface and sends them to the output FIFO

// Audio recording is functional, but will record a sawtooth wave with period 32 samples (250 Hz at 8 kHz sampling rate) on the simulator software
